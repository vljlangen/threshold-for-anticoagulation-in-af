...
################################################################################################
###                                                                                          ###
###                      PROBABILISTIC SENSITIVITY ANALYSIS SIMULATION                       ###
###                                                                                          ###
###        THIS SCRIPT SHOULD BE RUN 1000 SEPARATE TIMES (E.G., AS A SLURM ARRAY JOB)        ###
###                                                                                          ###
###                             BEST EXECUTED ON A SUPERCOMPUTER                             ###
###                                                                                          ###
###  RESULTS FROM ALL RUNS SHOULD BE MERGED AFTERWARD INTO A SINGLE DATA FRAME FOR ANALYSIS  ###
###                                                                                          ###
################################################################################################
################################################################################################
...
p_load(dplyr, gt, doParallel, doRNG)
...
decisionMaker <- function(isRisk = 1, policy = NULL, severity = NULL, months = 240, size = 10000, initial_state = NULL, seed = NULL, end.NOAC.after.bleeding = FALSE, age = 70, prob.sens = FALSE) {
  ...
  #remove excess death from double calculations of death rates due to event deaths
...
############################
## save prob sens results ##
############################

mean_qaly_with_noac <- vector(mode = "numeric", length = 101)
for (i in 1:101) {
  mean_qaly_with_noac[i] <- mean(sapply(simulated_cumqalys_with_noac[[i]], function(x) x$QALY))
}
mean_qaly_without_noac <- vector(mode = "numeric", length = 101)
for (i in 1:101) {
  mean_qaly_without_noac[i] <- mean(sapply(simulated_cumqalys_without_noac[[i]], function(x) x$QALY))
}



#################################################################
##         Save QALY summary results into a file (.rds)        ##
#################################################################

# Create the data frame
qaly_results <- data.frame(
  mean_qaly_with_noac,
  mean_qaly_without_noac,
  isRisk = seq(0, 10, 0.1)
)

# Get SLURM job and task IDs from environment
job_id <- as.integer(Sys.getenv("SLURM_JOB_ID"))
task_id <- as.integer(Sys.getenv("SLURM_ARRAY_TASK_ID"))

# Add job info into the data frame for traceability
qaly_results$job_id <- job_id
qaly_results$task_id <- task_id

# Get current timestamp formatted safely for filenames and logging
timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
qaly_results$timestamp <- timestamp

# Create a unique filename
output_file <- paste0("results_job", job_id, "_task", task_id, "_", timestamp, ".rds")

# Save the results
saveRDS(qaly_results, file = output_file)
